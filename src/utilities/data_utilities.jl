...
function try_to_get_integer(variable_value)
    try
        ...
        elseif variable_value isa String
            #parse as integer like"42"
            try
                return parse(Int, variable_value)
            catch
                #try to parse entire string as a float like "3.14", "1.2e2"
                try
                    float_val = parse(Float64, variable_value)
                    return round(Int, float_val)
                catch
                    #do partial-digit fallback.
        ...
        elseif variable_value isa AbstractArray && length(variable_value) > 0
            # For arrays, try the first numeric element
